...
const PhotoUploader = ({ addedPhotos, onChange }) => {
  ...
  function selectMainPhoto(ev, filename) {
    ev.preventDefault();
    const addedPhotosWithoutSelection = addedPhotos.filter((photo) => photo !== filename );
    const newAddedPhotos = [filename, ...addedPhotosWithoutSelection];
        ...
        {addedPhotos.length > 0 &&
          addedPhotos.map((link) => (
            <div className="h-32 flex   relative" key={link}>
              <img
                className="border rounded-2xl w-full object-cover "
                src={"http://localhost:5555/uploads/" + link}
                alt="phone/laptop/other"
              />
              <button
                onClick={(ev) => removePhoto(ev, link)}
                className=" absolute bottom-1 right-1 text-white  bg-black bg-opacity-40 p-1  rounded-2xl cursor-pointer"
              >
                <svg
              ...
              <button
                ...
                className=" absolute bottom-1 left-1 text-white  bg-black bg-opacity-40 p-1  rounded-2xl cursor-pointer"
              >
                {link === addedPhotos[0] && (
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
